...
if __name__ == "__main__":
    ...
    parser.add_argument("--backup", action="store_true", required=False)
    parser.add_argument("--prefix", type=str, required=False, help="Prefix for backup table. Required in case of backfill process.")
    # TODO: Add --prefix here for passing backup_prefix here
    ...
    # backup_prefix = f'_bkp_{datetime.strftime(datetime.now(),"%Y%m%d")}'
    location = tbl_details.get('Table').get('StorageDescriptor').get('Location')
    bkp_table = f'{table}{backup_prefix}'
    bkp_location = f'{location}{backup_prefix}'

    ## creating backfill table
    # generate table creation request
    if is_backup:
        request = _create_table_request(tbl_details, backup_prefix)

        # check if backup table already exists =>
        try:
            # check if table exists
            spark.sql(f"desc {bkp_table}")
            spark.sql(f"drop table {bkp_table}")
        except Exception as e:
            print(f"{bkp_table} doesn't exist.")
        ...
        # backup table path
        bucket, key = _get_bucket_key(bkp_location)
        s3_response = s3_client.put_object(Bucket=bucket, Key=key if key.endswith("/") else f'{key}/')
        if s3_response["ResponseMetadata"]["HTTPStatusCode"] == 200:
            print(f"{bkp_location} created successfully.")
    else:
        # reading data from original table
        src_df = spark.read.table(table)
        col_order = spark.sql(f"select * from {bkp_table} limit 1").columns

        # cast data types =>
        casted_df = _update_df_schema(spark, src_df, bkp_table)
        casted_df = _sync_data_type(spark, casted_df, bkp_table)
        casted_df.select(*col_order).write.insertInto(bkp_table, overwrite=True)

        print(f"{bkp_table} population is done.")
